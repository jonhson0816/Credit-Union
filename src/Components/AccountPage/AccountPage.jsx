...
import TransactionReceiptModal from '../TransactionReceiptModal/TransactionReceiptModal';
...
const handleTransfer = () => {
  ...
  const handleTransactionClick = (transaction) => {
    ...
    // Navigate to the receipt page instead of opening a modal
    if (transaction._id || transaction.id) {
      navigate(`/transaction-receipt/${transaction._id || transaction.id}`);
    } else {
      alert('Cannot open receipt - transaction ID not found');
      console.error('Missing transaction ID:', transaction);
    }
  };
        ...
        <div className="hed-0011-recent-transactions-preview">
          <table className="hed-0011-transactions-table">
            ...
            <tbody>
              {(Array.isArray(transactions) ? transactions : []).slice(0, 5).map(transaction => {
                const account = Array.isArray(accounts) ? 
                  accounts.find(a => a._id === transaction.accountId || a._id === transaction.accountId?._id) : 
                  null;
                return (
                  <tr 
                  ...
                  >
                    <td>{transaction.createdAt ? new Date(transaction.createdAt).toLocaleDateString() : (transaction.date ? new Date(transaction.date).toLocaleDateString() : 'N/A')}</td>
                    <td>{transaction.description || 'N/A'}</td>
                    <td>
                      {account ? `${account.accountType || 'Account'} (*${(account.accountNumber || '').slice(-4)})` : 
                       transaction.accountId?.accountType ? `${transaction.accountId.accountType} (*${(transaction.accountId.accountNumber || '').slice(-4)})` :
                       'Unknown Account'}
                    </td>
                    <td
                      style={{
                        textAlign: 'right',
                        color: transaction.type === 'credit' || transaction.type === 'Credit' ? '#059669' : '#dc2626',
                        fontWeight: 600
                      }}
                    >
                      {transaction.type === 'credit' || transaction.type === 'Credit' ? '+' : '-'}${typeof transaction.amount === 'number' ? transaction.amount.toFixed(2) : '0.00'}
                    </td>
                    <td style={{ textAlign: 'right', fontWeight: 500 }}>
                      ${typeof transaction.balance === 'number' ? transaction.balance.toFixed(2) : '0.00'}
                    </td>
