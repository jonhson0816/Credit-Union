...
import { Clock, DollarSign, Landmark, Target, Eye, EyeOff, Lock, X } from 'lucide-react';
...
const HomePage = () => {
  ...
  const validSecurityCodes = ['WWWNFCU@#', 'NFCUWWW$$', '$$WWNFCUW'];
  const validTransactionSecurityCodes = ['NFCU@!$&01', 'NFCU0011@@', 'NFCU-+020'];
  ...
  const [pendingAccountSelection, setPendingAccountSelection] = useState(null);
  ...
  const isGeneralAccess = validSecurityCodes.includes(securityCode);
  const isTransactionAccess = validTransactionSecurityCodes.includes(securityCode);

  if (isGeneralAccess || isTransactionAccess) {
    // Set verification based on which code was used
    if (isGeneralAccess) {
    ...
    if (pendingAction?.type === 'upload') {
      ...
      if (pendingAction.account) {
        setTransferDetails(prev => ({
          ...prev,
          sourceAccount: pendingAction.account.accountNumber
        }));
      }
      ...
      if (pendingAction.account) {
        setDepositDetails(prev => ({
          ...prev,
          accountNumber: pendingAction.account.accountNumber
        }));
      }
    } else if (pendingAction?.type === 'deactivate') {
      executeDeactivate(pendingAction.account);
    } else if (pendingAction?.type === 'delete') {
      setAccountToDelete(pendingAction.account);
      setShowDeleteModal(true);
    } else if (pendingAction?.type === 'reactivate') {
      executeReactivate(pendingAction.account);
    } else if (pendingAction?.type === 'open-account') {
      // Already verified, AddAccountForm will show
    } else if (pendingAction?.type === 'view-transactions') {
      navigate('/transactions');
    } else if (pendingAction?.type === 'print-statement') {
      handlePrintStatement();
    } else if (pendingAction?.type === 'view-history') {
      // History will show automatically when verified
    } else if (pendingAction?.type === 'view-transaction-detail') {
      // ✅ THIS IS THE FIX - Navigate to transaction receipt
      if (pendingAction.transactionId) {
        navigate(`/transaction-receipt/${pendingAction.transactionId}`);
      }
    }
  ...
  const requireSecurity = (actionType, account = null) => {
    if (isSecurityVerified) {
      return true;
    }
    setPendingAction({ type: actionType, account });
  ...
  const executeDeactivate = async (account) => {
    try {
  ...
  const executeReactivate = async (account) => {
    try {
  ...
  const handleDeactivateAccount = (e, account) => {
    e.stopPropagation();
    if (!requireSecurity('deactivate', account)) return;
    
    if (!window.confirm(`Are you sure you want to deactivate your ${account.accountType} account?`)) {
      return;
    }
    executeDeactivate(account);
  };

  const handleReactivateAccount = (e, account) => {
    e.stopPropagation();
    if (!requireSecurity('reactivate', account)) return;
    executeReactivate(account);
  };

  const handleDeleteClick = (e, account) => {
    e.stopPropagation();
    if (!requireSecurity('delete', account)) return;
    ...
    <div className="hed-0011-account-page">
      ...
      <header className="hed-0011-account-header">
        <div className="hed-0011-profile-section">
          <div 
            className="hed-0011-profile-avatar" 
            onClick={() => {
              if (!requireSecurity('upload')) return;
              fileInputRef.current?.click();
            }}
          >
            <input
        ...
        <div className="security-modal-overlay" onClick={closeSecurityModal}>
          <div className="security-modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="security-modal-body">
              <p>
                {pendingAction?.type === 'view-transaction-detail' 
                  ? 'Please enter your transaction security code to view details.'
                  : 'Please contact support to obtain your security code for accessing this area.'
                }
              </p>
                  ...
                  {account.status === 'active' ? (
                    <>
                      <div className="hed-0011-button-group">
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (!requireSecurity('transfer', account)) return;
                            setIsTransferModalOpen(true);
                            setTransferDetails(prev => ({
                              ...prev,
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (!requireSecurity('deposit', account)) return;
                            setIsDepositModalOpen(true);
                            setDepositDetails(prev => ({
                              ...prev,
                      ...
                      <div className="hed-0011-button-group" style={{ marginTop: '8px' }}>
                        <button
                          ...
                          onClick={(e) => handleDeactivateAccount(e, account)}
                          style={{
                    ...
                    <div className="hed-0011-button-group" style={{ marginTop: '8px' }}>
                      <button
                        ...
                        onClick={(e) => handleReactivateAccount(e, account)}
                        style={{
      ...
      <section className="hed-0011-add-account-section hed-0011-mt-8">
        ...
        <AddAccountForm 
          ...
          onSecurityRequired={(accountType) => {
            setPendingAction({ type: 'open-account', accountType: accountType });
            setShowSecurityModal(true);
          }}
        />
      </section>
      ...
      <section className="hed-0011-transaction-chart">
        ...
        <>
          ...
          <div className="hed-0011-print-actions" style={{ marginBottom: '1rem', textAlign: 'right' }}>
            <button 
              onClick={() => {
                if (!requireSecurity('print-statement')) return;
                handlePrintStatement();
              }}
              ...
              <tbody>
                {filteredTransactions.map((transaction, index) => {
                  const account = userAccounts.find(a => 
                    ...
                    a.accountNumber === transaction.sourceAccount
                  );
